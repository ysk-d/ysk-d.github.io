...
function search(query){return idx.search(query);};$(document).ready(function(){$('.post-new-comment').submit(function(){var form=this;$(form).addClass('loading');$('input[type="submit"]:enabled').addClass('hidden');$('input[type="submit"]:disabled').removeClass('hidden');var api='staticman3.herokuapp.com';var gitProvider='';var username='';var repo='';var branch='';$.ajax({type:$(this).attr('method'),url:['https:/',api,'v3/entry',gitProvider,username,repo,branch,'comments'].join('/'),data:$(this).serialize(),contentType:'application/x-www-form-urlencoded',success:function(data){showAlert('success');setTimeout(function(){clearForm()},3000);$(form).removeClass('loading');},error:function(err){console.log(err);showAlert('failed');$(form).removeClass('loading');}});return false;});function showAlert(msg){if(msg=='success'){$('.post-new-comment .submit-success').removeClass('hidden');$('.post-new-comment .submit-failed').addClass('hidden');}else{$('.post-new-comment .submit-success').addClass('hidden');$('.post-new-comment .submit-failed').removeClass('hidden');}
$('input[type="submit"]:enabled').removeClass('hidden');$('input[type="submit"]:disabled').addClass('hidden');}
function clearForm(){resetReplyTarget();$('.post-new-comment input').filter(function(){return this.name.match(/^fields\[.*\]$/);}).val('');$('.post-new-comment textarea').val('');$('.post-new-comment .submit-success').addClass('hidden');$('.post-new-comment .submit-failed').addClass('hidden');}
function resetReplyTarget(){$('.post-new-comment .post-reply-notice .post-reply-name').text('');$('.post-new-comment .post-reply-notice .post-comment-avatar').remove();$('.post-new-comment .post-reply-notice .post-reply-close-btn').remove();$('.post-new-comment .post-reply-notice').addClass('hidden');$('.post-new-comment input[type="hidden"]').filter(function(){return this.name.match(/^fields\[reply[a-zA-Z]*\]$/);}).val('');}
$('.post-comment').on('click','.post-comment-reply-btn',function(evt){resetReplyTarget();var cmt=$(evt.delegateTarget);var replyThread=cmt.find('.post-comment-threadID').text();$('.post-new-comment input[name="fields[replyThread]"]').val(replyThread);$('.post-new-comment input[name="fields[replyID]"]').val(cmt.attr("id"));authorTag=cmt.find('.post-comment-author');replyName=authorTag.text();$('.post-new-comment input[name="fields[replyName]"]').val(replyName);$('.post-new-comment .post-reply-notice').removeClass('hidden');$('.post-new-comment .post-reply-name').text(replyName);avatarTag=cmt.find('.post-comment-avatar');$('.post-new-comment .post-reply-arrow').after(avatarTag.clone());closeBtn=$("<a class='post-reply-close-btn button'></a>");$('.post-new-comment .post-reply-notice').append(closeBtn);});$('.post-new-comment .post-reply-notice').on('click','.post-reply-close-btn',function(){resetReplyTarget();});$('.post-new-comment input[type="reset"]').click(function(){clearForm();});});